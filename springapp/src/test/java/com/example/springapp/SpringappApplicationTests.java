...
public void testGetOneByAuthor() throws Exception {
    // Uncomment the lines below and add your test logic:
    String author = "Willam";
    mockMvc.perform(MockMvcRequestBuilders.get("/books/authors/{author}", author ))
            .andExpect(status().isOk());
}
...
public void testGetOneByYear() throws Exception {
    // Uncomment the lines below and add your test logic:
    int year = 1999;
    mockMvc.perform(MockMvcRequestBuilders.get("/books/year/{year}", year ))
            .andExpect(status().isOk());
}
